...
export default async function PainelLayout({ children }: { children: React.ReactNode }) {
  ...
  const [{ count: emChecagem }, { count: aguardandoSocio }] = await Promise.all([
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'stretch', minHeight: '100vh', background: 'var(--color-bg)' }}>
      <Sidebar usuario={usuario} contadores={{ emChecagem: emChecagem ?? 0, aguardandoSocio: aguardandoSocio ?? 0 }} />
